          ...
          <tr v-for="(exam, index) in paginatedExams" :key="index">
            ...
            <td class="py-4 border border-gray-300 table-cell">
              <button
                ...
                :class="{
                  'bg-green-500 hover:bg-green-600': exam.statusquestion === 1,
                  'bg-red-500 hover:bg-red-600': exam.statusquestion === 0,
                }"
                class="px-3 py-1 text-white rounded-md"
              >
                {{ exam.statusquestion === 1 ? "เปิด" : "ปิด" }}
              </button>
            </td>
...
export default {
  ...
  methods: {
    togglePublish(exam) {
      exam.statusquestion = 1 - exam.statusquestion; // เปลี่ยนสถานะของข้อสอบ (1 เป็น 0, 0 เป็น 1)
      this.updateExamStatus(exam); // เรียกใช้เมทอดเพื่อบันทึกการเปลี่ยนแปลง
    },

    async updateExamStatus(exam) {
      try {
        const response = await axios.put(
          `${import.meta.env.VITE_API_EXAM}/exam/update-exam/${exam._id}`,
          { statusquestion: exam.statusquestion }
        );
        if (response.status === 200) {
          Swal.fire("Success!", "Exam status updated successfully.", "success");
        } else {
